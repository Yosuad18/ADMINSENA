...
export default function ComputersEdit() {
    ...
    useEffect(() => { computersService.get(id).then(r => setForm({ number: r.data.number || '', brand: r.data.brand || '' })).catch(() => {}).finally(() => setLoading(false)); }, [id]);
    const handleChange = (e) => setForm(prev => ({ ...prev, [e.target.name]: e.target.value }));

    const handleSubmit = async (e) => {
        e.preventDefault(); setErrors({}); setSaving(true);
        try { await computersService.update(id, form); navigate('/admin/equipos'); } catch (err) { setErrors(err.response?.data?.errors || {}); } finally { setSaving(false); }
    };
    ...
    return (
        <div className="flex justify-center"><div className="w-full max-w-lg">
            <div className="bg-white rounded-xl shadow-sm p-6 border-t-4 border-sena-green">
                <h3 className="text-lg font-bold text-sena-dark mb-4"><i className="fas fa-edit text-sena-green mr-2"></i>Editar Equipo</h3>
